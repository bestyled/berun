...
import * as inquirer from 'react-dev-utils/inquirer'
import { sync as spawnSync } from 'react-dev-utils/crossSpawn'
import * as os from 'os'
import * as paths from '@berun/berun/src/options/paths'
...
inquirer
  ...
  .then(answer => {
    ...
    const files = folders.reduce<any[]>((files, folder) => {
      return files.concat(
        fs
    ...
    if (appPackage.devDependencies) {
      // We used to put @berun/scripts in devDependencies
    ...
    if (ownPath.indexOf(appPath) === 0) {
      try {
        // remove @berun/scripts and @berun/scripts binaries from app node_modules
        Object.keys(ownPackage.bin).forEach(binKey => {
    ...
    if (paths.useYarn) {
      ...
      if (process.platform === 'win32') {
        ...
        // Yarn is diligent about cleaning up after itself, but this causes the berun-scripts.cmd file
