...
module.exports = function(
  ...
  appPackage.devDependencies = { [ownPackageName]: appPackage.devDependencies[ownPackageName] }
  ...
  if (fs.existsSync(templatePath)) {
    ...
    fs.copySync(templatePath, appPath, (src) => { 
      return (src.indexOf('package.json') == -1)
    } );

    const templatePackagePath = path.resolve(templatePath, 'package.json');
    let devDependencies = require(templatePackagePath).devDependencies;
    ...
    let resolutions = require(templatePackagePath).resolutions;

    if (resolutions) {
      appPackage.resolutions = Object.assign(appPackage.resolutions || {}, resolutions);
    }

    let scripts = require(templatePackagePath).scripts;
