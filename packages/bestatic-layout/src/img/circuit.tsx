...
export const CircuitSvg: React.SFC<{
  theme: { colors: { primaryLight: string } }
}> = ({ theme }) => {
  const color =
    (theme && theme.colors && theme.colors.primaryLight) || '#3f51b5'

  // SVG by Steve Schoger under CC 4.0
  return (
    <svg
...
export const CircuitSvgStr = theme =>
  encodeURIComponent(renderToStaticMarkup(<CircuitSvg theme={theme} />))
