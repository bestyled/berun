...
and `runners` included for Webpack, Fuse-Box, Babel, Jest, ESLint, Prettier and many other front-end tools.
...
allows fine-grained reconfiguration using named plugins, for example:
...
We've demonstrated independence for all runners by also including Fuse-Box from the beginning, and we'll be adding
...
your project directories, as nearly everything is configured in a single `berun.config.js` file.
...
pipelines that can call any such tools.     We've lifted the light-weight but well-proven `Sparky` task manager 
from Fuse-Box and incorproated directly in BeRun (in its own dependency of course `@berun/sparky`)
