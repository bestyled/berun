...
test('default mode derived from production NODE_ENV', () => {
  ...
  const webpackConfig = berun().use(Webpack).webpack.mode(process.env.NODE_ENV).toConfig();
...
test('default mode derived from test NODE_ENV', () => {
  ...
  const webpackConfig = berun().use(Webpack).webpack.mode(process.env.NODE_ENV == 'production' ? 'production' : 'development').toConfig();
  expect(webpackConfig.mode).toBe('development');
});
...
test('undefined when trying to use a non-registered output', () => {
  expect(berun(Function.prototype).fake).toBe(undefined);
});

test('throws when trying to use a non-registered proxied method', () => {
  expect(() =>
    berun(Function.prototype).fake()).toThrow('fake is not a function');
});

test('exposes webpack output handler', () => {
  expect(() =>
    berun(Function.prototype).use(Webpack).webpack).not.toThrow();
});

test('exposes webpack config from output', () => {
  const handler = berun(Function.prototype).use(Webpack).webpack.toConfig();
  expect(typeof handler).toBe('object');
});

test('exposes webpack toConfig method', () => {
  expect(typeof berun(Function.prototype).use(Webpack).webpack.toConfig).toBe('function');
});
