...
module.exports = (berun, options = {}) => {
  var corePath = path.dirname(resolveCWD('@bestatic/core/dist/index.js'))

  if (!berun.options.cmd.endsWith(':static')) return
  ...
  const configPath = fs.existsSync(configLocal)
    ? configLocal
    : resolveCWD('@bestatic/core/src/config/bestatic.config.default.js')

  const opt = Object.assign({ configPath }, options)
