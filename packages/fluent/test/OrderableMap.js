import { FluentMap, OrderableMap } from '../src'

const Ordered = class Test extends OrderableMap {}
...
test('ordering before', () => {
  ...
  expect(map.values().map(o => o.values())).toEqual([['alpha'], ['beta']])
})
...
test('ordering before and after', () => {
  ...
  expect(map.values().map(o => o.values())).toEqual([
    ['alpha'],
    ['beta'],
    ['gamma']
  ])
