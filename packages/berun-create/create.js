...
const DEFAULT_TEMPLATE = process.env.REACT_SCRIPTS_TEMPLATE || '@berun/template-react';
...
const program = new commander.Command(packageJson.name)
  ...
  .on('--help', () => {
    ...
    console.log(
      `      ${chalk.cyan(
        'https://github.com/bestyled/berun/issues/new'
      )}`
    );
    console.log();
  })
...
function createApp(name, verbose, version, useNpm, template) {
  ...
  prerun(root, appName, version, verbose, originalDirectory, template || DEFAULT_TEMPLATE, useYarn);
}
...
function run(
  ...
  getPackageName(packageToInstall)
    ...
    .then(info => {
      ...
      return install(root, useYarn, allDependencies, verbose, isOnline, false).then(
        () => info
      );
    })
    .then(info => {
      ...
      return install(root, useYarn, [packageToInstall], verbose, isOnline, true).then(
        () => packageName
      );
    })
    .then(packageName => {
